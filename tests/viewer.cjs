...
const elements=new Map(), moving=new Set(['cv','hudL','hudR','stripToggle','dayCount','turnBars','turnTooltip']);
...
pipDocument={activeElement:null,head:element('head'),body:{classList:{add(){}},append(){moved=true}},addEventListener(){}};
...
const context=vm.createContext({console,document:parentDocument,location:{protocol:'http:',search:'?dev',hash:''},
  ...
  matchMedia:()=>({matches:false}),performance:{now:()=>1000},localStorage:{getItem(){return null},setItem(){}},
...
(async()=>{
  ...
  assert.equal(element('stripToggle').attributes['aria-pressed'],'true');
  ...
  run('setStrip(false)');assert.equal(element('stripToggle').attributes['aria-pressed'],'false');
  ...
  console.log('Viewer passed: PiP adoption/return, HUD and history updates, strip toggle, local moon/reset clock, four rendered moods.');
